...
public class txtres {
	...
	@Keyword
	def textresize (int textscaling, LoadTimeCorrector, step, run, device, timeStamp){
	int timed
	if(WebUI.waitForElementVisible(findTestObject('CuPaDl/Texts/Resize_text_rightside'), 3)==false){
		WebUI.click(findTestObject('CuPaDl/Texts/Resize_text'))
		WebUI.waitForElementVisible(findTestObject('CuPaDl/Texts/Resize_text_rightside'), 0)
		WebUI.delay(1)
	}
	WebUI.dragAndDropByOffset(findTestObject('CuPaDl/Texts/Resize_text_rightside'), WebUI.getViewportLeftPosition() +
